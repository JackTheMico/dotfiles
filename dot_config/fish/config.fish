...
function nyxhelp --description "NyxNiri Cheatsheet速查手册"
    ...
    switch "$section"
        case header
            echo ""
            set_color -o cyan
            echo "  [ NyxNiri Dotfiles 终端与桌面速查手册 ]"
            set_color normal
            echo ""
            return
        case cli
            set_color -o magenta
            echo "  [ NyxNiri CLI & 配置快照 ]"
            set_color normal
            set_color -o yellow
            echo -n "     nyxniri                 "
            set_color green
            ...
            set_color normal
            set_color -o yellow
            echo -n "     nyxniri doctor          "
            set_color green
            echo "-> 运行系统全要素诊断"
            set_color normal
            set_color -o yellow
            echo -n "     nyxniri snapshot [备注] "
            set_color green
            echo "-> 创建配置安全快照"
            set_color normal
            set_color -o yellow
            echo -n "     nyxniri rollback [序号] "
            set_color green
            echo "-> 恢复历史配置快照"
            set_color normal
            set_color -o yellow
        ...
        case proxy
            ...
            set_color normal
            set_color -o yellow
            echo -n "     proxy_on [端口/地址]    "
            set_color green
            echo "-> 开启代理 (默认 127.0.0.1:7890，Starship 实时显示)"
            set_color normal
            set_color -o yellow
            echo -n "     proxy_off               "
            set_color green
            echo "-> 关闭代理 (清除环境变量与 Prompt 标识)"
            set_color normal
            set_color -o yellow
            echo -n "     proxy_status            "
            set_color green
            echo "-> 检查代理连通性、延迟与公网 IP"
            set_color normal
            return
        case pkg
            set_color -o magenta
            ...
            echo -n "     up / update             "
            set_color green
            echo "-> 运行 shelly 一键全量更新"
            set_color normal
            set_color -o yellow
            echo -n "     in <包名>               "
            set_color green
            echo "-> 运行 shelly 安装软件包"
            set_color normal
            set_color -o yellow
            echo -n "     se                      "
            set_color green
            echo "-> 模糊搜索软件包并 fzf 交互安装"
            set_color normal
            set_color -o yellow
            echo -n "     un                      "
            set_color green
            echo "-> 模糊搜索已安装包并 fzf 交互卸载"
            set_color normal
            set_color -o yellow
            echo -n "     clean [--auto]          "
            set_color green
        ...
        case keys bind keybindings
            ...
            echo -n "     Mod + R / Mod + E       "
            set_color green
            echo "-> 启动 Noctalia Launcher / Nautilus"
            set_color normal
            set_color -o blue
            echo -n "     Mod + Q                 "
            set_color green
            ...
            echo -n "     Mod + L                 "
            set_color green
            echo "-> 锁定屏幕 (Noctalia Lock)"
            set_color normal
            set_color -o blue
            echo -n "     Mod + Shift + S / Print "
            set_color green
            echo "-> 交互式区域截图"
            set_color normal
            set_color -o blue
